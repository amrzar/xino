...
template <mmio_ok_v T>
[[gnu::always_inline]] inline T raw_read(mmio_addr_t addr) noexcept {
  return *ptr<T>(addr);
}
 ...
 */
template <mmio_ok_v T>
[[gnu::always_inline]] inline T read_relaxed(mmio_addr_t addr) noexcept {
 ...
 */
template <mmio_ok_v T>
[[gnu::always_inline]] inline T read_ordered(mmio_addr_t addr) noexcept {
...
[[gnu::always_inline]] inline std::uint16_t readw(mmio_addr_t addr) noexcept {
  return read_ordered<std::uint16_t>(addr);
}

// Ordered 32-bit MMIO read.
[[gnu::always_inline]] inline std::uint32_t readl(mmio_addr_t addr) noexcept {
